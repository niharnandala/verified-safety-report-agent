...
"""
section                       | wht it receives
------------------------------|------------------------------------------------------------
reporting_period              | start_date, end_date

narrative_summary             | case_volume (full), top_age_group, top_sex, top_country,
                              | top_reaction, top_serious_reaction, top_outcome, trend_hikes

summary_analysis_of_cases     | case_volume (full), demographics (full), outcome_counts (full),
                              | seriousness_breakdown (full), reporter_type (full)

reaction_ae_analysis          | top_reactions_all, top_reactions_serious,
                              | reaction_counts_by_age_group, reaction_counts_by_sex

serious_cases_15day           | alert_cases, non_alert_cases, alert_pct,
                              | alert_reactions, alert_outcomes

trends_observations           | trend_analysis full (median, mad, upper_limit, lower_limit,
                              | hikes, dips, granularity, counts)
        
history_of_actions            | fixed statement, no data, no analysis fn called

case_index_listing            | raw table: case_id, reaction, seriousness,
                              | reporting_date, country, outcome
